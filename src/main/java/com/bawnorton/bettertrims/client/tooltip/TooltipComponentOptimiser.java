...
public final class TooltipComponentOptimiser {
    ...
    private static ClientTooltipComponent flatten(ClientTooltipComponent component) {
        if (!(component instanceof CompositeComponent composite)) {
            if (component instanceof ConditionalComponent(ClientTooltipComponent nested, Supplier<Boolean> checker)) {
                return new ConditionalComponent(flatten(nested), checker);
            }
            return component;
        }
        ...
        for (ClientTooltipComponent child : composite.getComponents()) {
            switch (child) {
                case ConditionalComponent(ClientTooltipComponent nested, Supplier<Boolean> checker) -> builder.component(new ConditionalComponent(flatten(nested), checker));
    ...
    private static ClientTooltipComponent mergeTextTooltips(ClientTooltipComponent component) {
        if (!(component instanceof CompositeComponent composite)) {
            if (component instanceof ConditionalComponent(ClientTooltipComponent nested, Supplier<Boolean> checker)) {
                return new ConditionalComponent(mergeTextTooltips(nested), checker);
            }
            return component;
        }
        ...
        for (ClientTooltipComponent child : composite.getComponents()) {
            switch (child) {
                ...
                case ConditionalComponent(ClientTooltipComponent nested, Supplier<Boolean> checker) -> {
                    merger.flush(builder);
                    builder.component(new ConditionalComponent(mergeTextTooltips(nested), checker));
                }
                default -> {
                    merger.flush(builder);
    ...
    private static ClientTooltipComponent flattenHorizontal(ClientTooltipComponent component) {
        if (!(component instanceof CompositeComponent composite)) {
            if (component instanceof ConditionalComponent(ClientTooltipComponent nested, Supplier<Boolean> checker)) {
                return new ConditionalComponent(flattenHorizontal(nested), checker);
            }
            return component;
        }
