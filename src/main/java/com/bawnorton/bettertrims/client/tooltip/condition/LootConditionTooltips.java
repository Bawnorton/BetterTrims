...
public interface LootConditionTooltips {
    static CompositeContainerComponent getTooltip(ClientLevel level, Font font, LootItemCondition condition) {
        CompositeContainerComponent.Builder root = new CompositeContainerComponent.Builder()
            .translate("bettertrims.tooltip.condition.if", Styler::condition)
            .space();
        CompositeContainerComponent.Builder switchingTooltip = CompositeContainerComponent.builder()
            .component(new ConditionalComponent(
                new ClientTextTooltip(
                    Styler.condition(Component.translatable(
        ...
        if (conditionComponent.isOneLine() && conditionComponent.getMaxWidth(font) < Minecraft.getInstance().getWindow().getScreenWidth() / 4) {
            return root.component(conditionComponent).build();
        } else {
            return root.component(switchingTooltip.component(new ConditionalComponent(conditionComponent, Screen::hasShiftDown)).build()).build();
        }
    }
    ...
    Map<LootItemConditionType, LootTooltipAdder<?>> TOOLTIPS = Util.make(
        new HashMap<>(), map -> {
            map.put(
                LootItemConditions.INVERTED,
                (ClientLevel level, InvertedLootItemCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.RANDOM_CHANCE,
                (level, condition, parentBuilder, state) -> {
                ...
                (ClientLevel level, LootItemRandomChanceWithEnchantedBonusCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
                    ...
                    parentBuilder.component(CompositeContainerComponent.builder()
                        ...
                        .translate(
                            ...
                            Styler::condition,
                            Styler.number(Component.literal("%.0f".formatted(condition.unenchantedChance() * 100) + "%"))
                        )
                        .build());
                }
            );
            ...
            map.put(
                LootItemConditions.ENTITY_SCORES,
                (ClientLevel level, EntityHasScoreCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
                    Component target = Styler.target(Component.translatable("bettertrims.tooltip.condition.entity_properties.%s.%s".formatted(
                        state.key(),
                        condition.entityTarget().getSerializedName()
                    )));
                    ...
                    builder.cycle(scoreCycler -> scores.forEach((score, range) -> {
                        ...
                        List<Component> minComps = min == null ? null : numberCompFromNumberProvider(min, formatter, UnaryOperator.identity());
                        List<Component> maxComps = max == null ? null : numberCompFromNumberProvider(max, formatter, UnaryOperator.identity());
            ...
            map.put(
                LootItemConditions.MATCH_TOOL,
                (ClientLevel level, MatchTool matchTool, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.TABLE_BONUS,
                (ClientLevel level, BonusLevelTableCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
                    ...
                    CompositeContainerComponent.Builder builder = CompositeContainerComponent.builder();
                    builder.translate("bettertrims.tooltip.condition.table_bonus.%s".formatted(state.key()), Styler::condition, enchantmentName)
                        .space()
                        .cycle(cycler -> {
                            for (int i = 0; i < values.size(); i++) {
                                float value = values.get(i);
                                String formattedValue = value % 1 == 0 ? "%.0f".formatted(value) : value * 10 % 1 == 0 ? "%.1f".formatted(value) : "%.2f".formatted(value);
                                MutableComponent levelComp = Styler.value(Component.translatable("enchantment.level." + (i + 1))).append(": ");
                                MutableComponent valueComp = Styler.number(Component.literal(formattedValue));
            ...
            map.put(
                LootItemConditions.SURVIVES_EXPLOSION,
                (ClientLevel level, ExplosionCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.LOCATION_CHECK,
                (ClientLevel level, LocationCheck locationCheck, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.WEATHER_CHECK,
                (ClientLevel level, WeatherCheck weatherCheck, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.REFERENCE,
                (ClientLevel level, ConditionReference conditionReference, CompositeContainerComponent.Builder parentBuilder, State state) -> {
            ...
            map.put(
                LootItemConditions.TIME_CHECK,
                (ClientLevel level, TimeCheck timeCheck, CompositeContainerComponent.Builder parentBuilder, State state) -> {
                    ...
                    Function<Float, String> formatter = ticks -> {
                        ...
                        if (hours == 0) hours = 12;
                        if (days > 0) {
                            return "%d:%02d%s + %d Days".formatted(hours, minutes, ampm, days);
                        } else {
                            return "%d:%02d%s".formatted(hours, minutes, ampm);
                        }
                    };

                    List<Component> minComps = min == null ?
                        numberCompFromNumberProvider(new ConstantValue(0), formatter, UnaryOperator.identity())
                        : numberCompFromNumberProvider(min, formatter, UnaryOperator.identity());
                    List<Component> maxComps = max == null ?
                        numberCompFromNumberProvider(new ConstantValue(24000), formatter, UnaryOperator.identity())
            ...
            map.put(
                LootItemConditions.VALUE_CHECK,
                (ClientLevel level, ValueCheckCondition condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
                    ...
                    List<Component> providerComps = numberCompFromNumberProvider(provider, formatter, UnaryOperator.identity());
                    List<Component> minComps = min == null ? null : numberCompFromNumberProvider(min, formatter, UnaryOperator.identity());
                    List<Component> maxComps = max == null ? null : numberCompFromNumberProvider(max, formatter, UnaryOperator.identity());
                    CompositeContainerComponent.Builder builder = CompositeContainerComponent.builder();
                    builder.translate("bettertrims.tooltip.condition.value_check.%s".formatted(state.key()), Styler::condition, providerComps)
                        .space();
                    if (minComps != null && maxComps != null) {
            ...
            map.put(
                DimensionCheck.TYPE,
                (ClientLevel level, DimensionCheck condition, CompositeContainerComponent.Builder parentBuilder, State state) -> {
